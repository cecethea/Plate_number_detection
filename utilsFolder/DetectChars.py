...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:  # for each possible plate, this is a big for loop that takes up most
        # of the function
        ...
        # find all possible chars in the plate, this function first finds all contours, then only includes contours
        # that could be chars (without comparison to other chars yet)
...
def checkIfPossibleChar(possibleChar):
    ...
    if (possibleChar.intBoundingRectArea > MIN_PIXEL_AREA and
            possibleChar.intBoundingRectWidth > MIN_PIXEL_WIDTH and possibleChar.intBoundingRectHeight
            > MIN_PIXEL_HEIGHT and MIN_ASPECT_RATIO < possibleChar.fltAspectRatio < MAX_ASPECT_RATIO):
        return True
    else:
        return False
    # end if
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:  # for each possible char in the one big list of chars
        ...
        if len(
                listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS:  # if current possible list of matching chars is
        ...
        for recursiveListOfMatchingChars in recursiveListOfListsOfMatchingChars:  # for each list of matching chars
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:  # for each char in big list
        if possibleMatchingChar == possibleChar:  # if the char we attempting to find matches for is the exact same
            # char as the char in the big list we are currently checking then we should not include it in the list of
            # matches b/c that would end up double including the current char
...
def angleBetweenChars(firstChar, secondChar):
    ...
    if fltAdj != 0.0:  # check to make sure we do not divide by zero if the center X positions are equal,
